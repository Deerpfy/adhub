# Code of Conduct

## Our Commitment

We are committed to providing a friendly, safe, and welcoming environment for all contributors, regardless of experience level, background, or identity.

## Expected Behavior

- **Be respectful** - Treat everyone with respect and consideration
- **Be constructive** - Provide helpful feedback and accept it gracefully
- **Be collaborative** - Work together towards common goals
- **Be inclusive** - Welcome newcomers and help them get started
- **Be patient** - Remember that everyone was a beginner once
- **Be professional** - Focus on what is best for the community and project

## Community Guidelines

1. **Use welcoming language** - Be kind and courteous in all interactions
2. **Respect different viewpoints** - Disagreement is fine, disrespect is not
3. **Accept feedback gracefully** - We all learn from constructive criticism
4. **Focus on the project** - Keep discussions relevant and productive
5. **Support each other** - Help fellow contributors when you can

## Scope

This Code of Conduct applies to all project spaces, including:
- GitHub repository (issues, PRs, discussions)
- Project communication channels
- Events and meetups related to the project

## Reporting Issues

If you experience or witness behavior that violates this Code of Conduct:

1. **Contact the maintainer** - Reach out to [@Deerpfy](https://github.com/Deerpfy)
2. **Provide details** - Describe what happened, when, and where
3. **Confidentiality** - All reports will be handled discreetly

## Enforcement

Maintainers are responsible for clarifying and enforcing standards of acceptable behavior. Actions may include:

- A friendly reminder about guidelines
- Temporary or permanent exclusion from the project

## Attribution

This Code of Conduct is adapted from the [Contributor Covenant](https://www.contributor-covenant.org/), version 2.1.

---

Thank you for helping make this a welcoming community for everyone!
