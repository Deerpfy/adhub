/**
 * AdHub YouTube Downloader v5.7 - Popup Script (Firefox)
 * Hybridni rezim: zakladni stahovani + volitelne rozsirene funkce
 * Firefox verze - pouziva browser.* API
 */
...
(function() {
  ...
  async function init() {
    console.log('[Popup] Inicializace v5.7 (Firefox)');
  ...
  function displayExtensionId() {
    // Firefox: browser.runtime.id vraci addon ID
    ...
    if (settingsExtIdEl) {
      settingsExtIdEl.textContent = extensionId;
    }
  }
  ...
  function showNativeHostError(errorMsg) {
    ...
    if (errorBox) {
      ...
      if (errorText) {
        if (errorMsg && errorMsg.includes('No such native application')) {
          errorText.innerHTML = '<strong>Native Host neni nainstalovan!</strong><br>Spustte instalator pro Firefox.';
        } else if (errorMsg && errorMsg.includes('forbidden')) {
          errorText.innerHTML = '<strong>Registry nesedi s extension ID!</strong><br>Spustte znovu instalator pro Firefox.';
  ...
  async function autoDetectPaths() {
    ...
    // Nabidnout typicke cesty pro Linux/Mac
    const ytdlpInput = $('#ytdlp-path');
    const ffmpegInput = $('#ffmpeg-path');

    ytdlpInput.placeholder = '/usr/local/bin/yt-dlp nebo ~/.local/bin/yt-dlp';
    ffmpegInput.placeholder = '/usr/local/bin/ffmpeg nebo ~/.local/bin/ffmpeg';
  ...
  async function copyExtensionId() {
    try {
      await navigator.clipboard.writeText(browserAPI.runtime.id);
      showToast('Extension ID zkopirovano!');
    } catch (e) {
      // Fallback
      const textArea = document.createElement('textarea');
      textArea.value = browserAPI.runtime.id;
  ...
  async function downloadInstaller(platform) {
    ...
    try {
      ...
      try {
        const apiUrl = `https://api.github.com/repos/Deerpfy/adhub/commits?path=projects/youtube-downloader/native-host/adhub-install-firefox.${ext}&per_page=1`;
      ...
      const filename = `adhub-install-firefox_${commitHash}.${ext}`;
      ...
      if (platform === 'windows') {
        ...
        showToast(`Stazeno ${commitHash}! ./adhub-install-firefox_${commitHash}.sh`);
      }
    } catch (e) {
