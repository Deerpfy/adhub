...
const PLUGIN_PATH = 'projects/cardharvest/plugin';
const SERVICE_PATH = 'projects/cardharvest/native-host';
...
function handleExtensionMessage(event) {
    ...
    if (event.data?.source !== 'cardharvest-extension') return;
...
function handleExtensionConnected(data) {
    ...
    if (data.serviceRunning) {
        ...
        updateConnectionStatus('disconnected', 'CardHarvest Service neběží');
        showExtensionRequired();
    }
}
...
async function handleDownloadPlugin() {
    ...
    try {
        ...
        const zip = new JSZip();
        const pluginFolder = zip.folder('cardharvest-plugin');
        ...
        a.download = `cardharvest-plugin-${commitId}.zip`;
...
async function handleDownloadService() {
    ...
    try {
        ...
        const zip = new JSZip();
        const serviceFolder = zip.folder('cardharvest-service');

        const excludeFiles = ['dist', 'installer-unix.sh', 'installer-windows.ps1', 'install.bat', 'install.sh', 'com.adhub.cardharvest.json'];
        ...
        const a = document.createElement('a');
        a.href = downloadUrl;
        a.download = 'cardharvest-service.zip';
