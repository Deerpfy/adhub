...
// Data directory
const DATA_DIR = path.join(process.env.APPDATA || process.env.HOME, '.adhub-cardharvest');
...
function startServer() {
    if (checkAlreadyRunning()) {
        console.log(`CardHarvest Service is already running.`);
    ...
    httpServer.on('error', (err) => {
        if (err.code === 'EADDRINUSE') {
            ...
            console.error(`CardHarvest Service may already be running.`);
            removePidFile();
            process.exit(1);
        }
