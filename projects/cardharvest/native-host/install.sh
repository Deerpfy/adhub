...
echo "============================================"
echo "CardHarvest - Native Host Installer (Linux/macOS)"
echo "============================================"
echo
...
HOST_PATH="$SCRIPT_DIR/cardharvest-host.js"
...
if [ -d "$(dirname "$CHROME_DIR")" ] || [[ "$OSTYPE" == "darwin"* ]]; then
    mkdir -p "$CHROME_DIR"
    echo "$MANIFEST_CONTENT" > "$CHROME_DIR/com.adhub.cardharvest.json"
    echo "[OK] Chrome"
fi
...
if [ -d "$(dirname "$CHROMIUM_DIR")" ]; then
    ...
    echo "$MANIFEST_CONTENT" > "$CHROMIUM_DIR/com.adhub.cardharvest.json"
    echo "[OK] Chromium"
fi
...
if [ -d "$(dirname "$BRAVE_DIR")" ]; then
    mkdir -p "$BRAVE_DIR"
    echo "$MANIFEST_CONTENT" > "$BRAVE_DIR/com.adhub.cardharvest.json"
    echo "[OK] Brave"
fi
...
if [ -d "$(dirname "$EDGE_DIR")" ]; then
    mkdir -p "$EDGE_DIR"
    echo "$MANIFEST_CONTENT" > "$EDGE_DIR/com.adhub.cardharvest.json"
    echo "[OK] Edge"
fi
