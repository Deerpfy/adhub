...
echo ============================================
echo CardHarvest - Native Host Installer (Windows)
echo ============================================
echo.
...
set SCRIPT_DIR=%~dp0
set HOST_PATH=%SCRIPT_DIR%cardharvest-host.js

:: Check if host exists
if not exist "%HOST_PATH%" (
    echo [ERROR] cardharvest-host.js nenalezen!
    pause
    exit /b 1
)
...
set MANIFEST_PATH=%SCRIPT_DIR%com.adhub.cardharvest.json
...
(
echo {
echo     "name": "com.adhub.cardharvest",
echo     "description": "CardHarvest Native Messaging Host pro AdHUB",
...
:: Chrome
reg add "HKCU\Software\Google\Chrome\NativeMessagingHosts\com.adhub.cardharvest" /ve /t REG_SZ /d "%MANIFEST_PATH%" /f >nul 2>&1
if %errorlevel% equ 0 echo [OK] Chrome

:: Edge
reg add "HKCU\Software\Microsoft\Edge\NativeMessagingHosts\com.adhub.cardharvest" /ve /t REG_SZ /d "%MANIFEST_PATH%" /f >nul 2>&1
if %errorlevel% equ 0 echo [OK] Edge

:: Brave
reg add "HKCU\Software\BraveSoftware\Brave-Browser\NativeMessagingHosts\com.adhub.cardharvest" /ve /t REG_SZ /d "%MANIFEST_PATH%" /f >nul 2>&1
