#!/bin/bash
# CardHarvest Service - One-Click Installer pro Linux/macOS
...
echo "============================================"
echo "CardHarvest Service Installer v$VERSION"
echo "============================================"
echo ""
echo "Tento instalátor nainstaluje CardHarvest Service."
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    OS="macos"
    INSTALL_DIR="$HOME/.adhub-cardharvest"
    BINARY_NAME="cardharvest-service-macos"
else
    OS="linux"
    INSTALL_DIR="$HOME/.adhub-cardharvest"
    BINARY_NAME="cardharvest-service-linux"
fi
...
# Stažení executable
echo ""
echo "[3/4] Stahuji CardHarvest Service..."
DOWNLOAD_URL="https://github.com/$REPO/releases/download/cardharvest-v$VERSION/$BINARY_NAME"
...
if curl -fsSL "$DOWNLOAD_URL" -o "$SERVICE_PATH" 2>/dev/null; then
    ...
    if [ -f "$LOCAL_BUILD" ]; then
        ...
        if command -v node &> /dev/null; then
            ...
            if [ -f "$SCRIPT_DIR/cardharvest-service.js" ]; then
                cp "$SCRIPT_DIR/cardharvest-service.js" "$INSTALL_DIR/"
...
#!/bin/bash
cd "$(dirname "$0")"
exec node cardharvest-service.js "$@"
WRAPPER
                chmod +x "$SERVICE_PATH"
                echo "      Nainstalováno pomocí Node.js"
            else
                echo "      [CHYBA] cardharvest-service.js nenalezen!"
                exit 1
            fi
        else
...
if [[ "$OS" == "linux" ]]; then
    ...
    DESKTOP_FILE="$AUTOSTART_DIR/cardharvest-service.desktop"

    mkdir -p "$AUTOSTART_DIR"
    cat > "$DESKTOP_FILE" << EOF
[Desktop Entry]
Type=Application
Name=CardHarvest Service
Exec=$SERVICE_PATH
Hidden=false
NoDisplay=true
X-GNOME-Autostart-enabled=true
Comment=CardHarvest background service
EOF
    echo ""
...
if [[ "$OS" == "macos" ]]; then
    ...
    PLIST_FILE="$LAUNCH_AGENTS_DIR/com.adhub.cardharvest.plist"
...
<dict>
    <key>Label</key>
    <string>com.adhub.cardharvest</string>
...
echo "============================================"
echo ""
echo "CardHarvest Service běží na: ws://127.0.0.1:17532"
echo "Status endpoint: http://127.0.0.1:17532/status"
echo ""
echo "Otevřete rozšíření CardHarvest v prohlížeči."
echo ""
