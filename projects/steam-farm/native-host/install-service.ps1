# Steam Farm Service - One-Click Installer pro Windows
...
Write-Host "Steam Farm Service Installer v$VERSION" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "Tento instalator nainstaluje Steam Farm Service." -ForegroundColor White
...
# Stažení executable
Write-Host ""
Write-Host "[3/4] Stahuji Steam Farm Service..." -ForegroundColor White
$DownloadUrl = "https://github.com/$REPO/releases/download/steam-farm-v$VERSION/steam-farm-service-win.exe"
...
try {
    ...
    $LocalBuild = Join-Path $PSScriptRoot "dist\steam-farm-service-win.exe"
    if (Test-Path $LocalBuild) {
        ...
        if ($nodeCheck) {
            $serviceJs = Join-Path $PSScriptRoot "steam-farm-service.js"
            $packageJson = Join-Path $PSScriptRoot "package.json"

            if (Test-Path $serviceJs) {
                Copy-Item $serviceJs "$InstallDir\steam-farm-service.js" -Force
...
"@
                ...
                Write-Host "      [CHYBA] steam-farm-service.js nenalezen!" -ForegroundColor Red
                exit 1
            }
        } else {
...
$ShortcutPath = "$StartupFolder\Steam Farm Service.lnk"
...
try {
    ...
    $Shortcut.Description = "Steam Farm Service"
    $Shortcut.Save()
...
Write-Host "============================================" -ForegroundColor Green
Write-Host ""
Write-Host "Steam Farm Service bezi na: ws://127.0.0.1:17532" -ForegroundColor White
Write-Host "Status endpoint: http://127.0.0.1:17532/status" -ForegroundColor White
Write-Host ""
Write-Host "Otevrete rozsireni Steam Farm v prohlizeci." -ForegroundColor Yellow
Write-Host ""
