...
class DocBookEditor {
    ...
    init() {
        ...
        if (!this.textarea || !this.preview) {
            console.error('[DocBook Editor] Editor elements not found');
            return;
        }
        ...
        console.log('[DocBook Editor] Initialized');
    }
    ...
    setMode(mode) {
        ...
        // Save preference
        window.DocBookDB.SettingsDB.set('editorMode', mode);
    }
...
`;
    ...
    async save() {
        ...
        try {
            ...
            await window.DocBookDB.HistoryDB.save(
                this.currentPage.id,
                content,
            ...
            // Update page
            await window.DocBookDB.PageDB.update(this.currentPage.id, { content });
            ...
            // Show toast
            window.DocBookApp?.showToast('Uloženo', 'success');

            console.log('[DocBook Editor] Page saved');
        } catch (error) {
            console.error('[DocBook Editor] Save error:', error);
            window.DocBookApp?.showToast('Chyba při ukládání', 'error');
        }
    }
...
window.DocBookEditor = new DocBookEditor();

console.log('[DocBook] Editor module loaded');
