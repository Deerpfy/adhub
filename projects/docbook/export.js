...
class DocBookExport {
    ...
    async export(scope = 'all', format = 'json', context = {}) {
        try {
            ...
            console.error('[DocBook Export] Export error:', error);
            throw error;
        }
    }
    ...
    async getSpaceData(spaceId) {
        const space = await window.DocBookDB.SpaceDB.getById(spaceId);
        if (!space) {
    ...
    exportAsJSON(data) {
        ...
        const filename = `docbook-export-${this.getTimestamp()}.json`;
    ...
    async exportAsMarkdown(data, scope) {
        ...
        let combinedContent = `# DocBook Export\n\n`;
        ...
        for (const space of data.spaces) {
            ...
            const tree = window.DocBookDB.PageDB.buildTree(spacePages, null);
        ...
        const filename = `docbook-export-${this.getTimestamp()}.md`;
...
`;
    ...
    async exportAsHTML(data, scope) {
        ...
        const filename = `docbook-export-${this.getTimestamp()}.html`;
...
`;
        ...
        <footer class="footer">
            <p>Generated by DocBook - Offline Documentation Platform</p>
        </footer>
    </div>
</body>
</html>`;
    ...
    async importJSON(content) {
        try {
            ...
            if (!data.version || !data.spaces) {
                throw new Error('Invalid DocBook backup format');
            }
            ...
            await window.DocBookDB.DBUtils.importAll(data);
            ...
            console.error('[DocBook Import] JSON import error:', error);
...
window.DocBookExport = new DocBookExport();

console.log('[DocBook] Export module loaded');
