/**
 * Scribblix - Search Module
 * Uses FlexSearch for fast fulltext search
 * Apache 2.0 License
 */

class ScribblixSearch {
    constructor() {
        this.index = null;
    ...
    createIndex() {
        if (typeof FlexSearch === 'undefined') {
            console.warn('[Scribblix Search] FlexSearch not loaded yet');
            return null;
        }
    ...
    async init() {
        try {
            ...
            if (!this.index) {
                console.warn('[Scribblix Search] Nelze vytvořit index, FlexSearch není dostupný');
                return;
            }
            ...
            console.log(`[Scribblix Search] Indexováno ${pages.length} stránek`);
        } catch (error) {
            console.error('[Scribblix Search] Chyba inicializace:', error);
        }
    }
    ...
    addPage(page) {
        ...
        if (!this.index) {
            console.warn('[Scribblix Search] Index není dostupný');
            return;
        }
        ...
        try {
            ...
            console.error('[Scribblix Search] Chyba při přidávání stránky:', error);
        }
    }
    ...
    updatePage(page) {
        ...
        try {
            ...
            console.error('[Scribblix Search] Chyba při aktualizaci stránky:', error);
        }
    }
    ...
    removePage(pageId) {
        ...
        try {
            ...
            console.error('[Scribblix Search] Chyba při odstraňování stránky:', error);
        }
    }
    ...
    async search(query, options = {}) {
        ...
        if (!this.index) {
            return await window.ScribblixDB.PageDB.search(query);
        }
        ...
        try {
            ...
            console.error('[Scribblix Search] Chyba vyhledávání:', error);
...
window.ScribblixSearch = new ScribblixSearch();

console.log('[Scribblix] Search modul načten');
